...
async function activeSession(sessionId) {
  ...
  const row = await trySupabase('approval/session', () => supabase.from('agent_sessions').select('created_at').eq('id', sessionId).single(), null);
...
export async function createApproval(sessionId, proposedAction) {
  if (!(await activeSession(sessionId))) return null;
...
export async function decide(approvalId, decision, decidedBy = 'human', sessionId = null) {
  ...
  if (!(await activeSession(sessionId))) return { status: 'forbidden' };
  ...
  if (!sessionId || appr.session_id !== sessionId || appr.proposed_action?.tool !== 'createLead' || Date.now() - new Date(appr.created_at).getTime() >= 86400000) {
