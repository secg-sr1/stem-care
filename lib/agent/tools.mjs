...
import { validateContact } from '../http/security.mjs';
...
async function knowledgeSearch({ query, k = 5 }, { language = 'es' } = {}) {
  ...
  try {
    ...
    console.error('[knowledgeSearch] unavailable; using static facts');
    return staticFacts(language);
  }

  if (inst.error || gen.error) {
    console.error('[knowledgeSearch] lookup failed; using static facts');
    return staticFacts(language);
  }
...
export const TOOL_SCHEMAS = TOOLS.filter(t => t.name !== 'sendEmail').map((t) => t.schema);
