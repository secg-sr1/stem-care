...
export async function limitRequest(req, bucket, limit = 15) {
  if (stateless()) {
    // Enable only after deploying a Vercel WAF rule for /api/* on every host.
    // The edge counter is shared across serverless instances, unlike a Map.
    if (!process.env.VERCEL || process.env.STEMCARE_EDGE_RATE_LIMIT !== 'true') {
      throw new HttpError(503, 'Production request protection is not configured.');
    }
    return;
  }
